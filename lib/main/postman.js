...
exports.get = function(caller, path, params, onErrorBlock, onSuccessBlock){
    ...
    request(url, options, function (error, response, body) {
        postmanCallback(error, response, body, url, params, onErrorBlock, onSuccessBlock)
    });
};
...
exports.put = function(caller, path, params, onErrorBlock, onSuccessBlock){
    var url = caller_config(caller)['url'] + path;
    var options = {'content-type': 'application/json',  body: JSON.stringify(params)};
...
function postmanCallback(error, response, body, url, params, onErrorBlock, onSuccessBlock){
    if (!error && response.statusCode == 200) {
        ...
        onErrorBlock({url:url, params: params, error: error});
    }
}
